...
routes.post('/updateProfileDetails', (req, res) => {
    var payload = {
            email: req.body.emailId,
            userDetails: req.body.updateProfileDetails,
        }
    userService.updateProfileDetails(payload)
    .then( results => {
        res.status(200).json({ success: true, message: "Updated details", payload: payload });
    })
    .catch( err => {
        res.status(400).json({ success: false, message: err.message, payload: null });
    })
